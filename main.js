...
// Check for updates to the contents.json every 5 minutes
var refreshTimer = window.setInterval(updateContentsJson, 5 * 60 * 1000);
function updateContentsJson() {
  // Fetch latest contents.json file
  $.ajax({ url: "https://raw.githubusercontent.com/dkhamsing/open-source-ios-apps/master/contents.json", dataType: "text", cache: false }).done(function(d) {
    var newHash = md5(d);
    // If the hash of the old contents.json is the same as this, then we don't need to update
    if(newHash == contentsJsonHash) return;
    else contentsJsonHash = newHash;
    data = JSON.parse(d);

    // Make a copy of appHashes to keep track of which have not been found
    var untrackedHashes = JSON.parse(JSON.stringify(appHashes));
    // Keep track of new hashes in the updated contents.json
    var newProjects = [];
    for(var i = 0; i < data.projects.length; i++) {
      var currentProject = data.projects[i];
      var currentProjectHash = md5(JSON.stringify(currentProject));

      var index = untrackedHashes.indexOf(currentProjectHash);
      if(index >= 0) {
        // App is unchanged
        untrackedHashes.splice(index, 1);
      }
      else {
        // App is new or modified
        addApp(currentProject);
      }
    }

    // Delete outdated apps
    for(var i = 0; i < untrackedHashes.length; i++) {
      $("#main-content .list-group-item[data-hash=" + untrackedHashes[i] + "]").remove();
      appHashes.splice(appHashes.indexOf(untrackedHashes[i]), 1);
    }
...
$(document).ready(function() {
  ...
  { name: "js-md5", critical: true, url: "https://cdn.jsdelivr.net/js-md5/0.4.1/md5.min.js", sri: "sha384-C9y2PZqYbk8Pf3jN4M6Akmv3vldZRalDZCmTvbk6/RpRCyx/Di+zhhgP09WFZ9QD" }];
  ...
  // Fetch latest contents.json file
	$.ajax({ url: "https://raw.githubusercontent.com/dkhamsing/open-source-ios-apps/master/contents.json", dataType: "text", cache: false }).always(function(d) {
    loadedResources++;
    updateResourcesProgress();
    data = d;
  }).fail(function() {
